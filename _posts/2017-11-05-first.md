...
The plot shows **Loss vs. Learning rate** for the dataset. Now its easy to choose an optimal range for learning rate before the curve flattens. 

So simple -once discovered- that surprises how revolutionary it is. It has the potential to save a huge amount of hours and  computational resources as soon as it becomes widespread practice. Aditionally to realizing this I also thought about how insightful it would be to **use the tool  to visualize the famous relationship between learning rate and batch size.**
